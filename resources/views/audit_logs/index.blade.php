        ...
        <div class="card card-responsive">
            <div class="card-header bg-info text-white d-flex flex-wrap justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2 mt-2 mt-md-0">
                    ...
                    <button class="btn btn-sm btn-light" data-manual-refresh="audit-logs-table-container" data-refresh-url="{{ route('audit-logs.index') }}">
                        <i class="fas fa-sync-alt"></i> Refresh Now
                    </button>
                    <x-ui.button variant="primary" href="{{ route('audit-logs.create') }}" class="btn-sm">Add Audit Log</x-ui.button>
                </div>
            </div>
            <div class="card-body" id="audit-logs-table-container" data-auto-refresh="true" data-refresh-url="{{ route('audit-logs.index') }}" data-refresh-target="audit-logs-table-container">
                @if (session('success'))
                ...
                <tbody>
                    @foreach ($logs as $log)
                        <tr>
                            ...
                            <td class="hide-mobile">{{ $log->created_at ? $log->created_at->format('Y-m-d H:i') : '' }}</td>
                            <td>
